...
public class JupyterhubController {
    ...
    public static final String COMPONENT_JUPYTERHUB = "jupyterhub";
    ...
    public String create(@RequestParam Map<String, String> params) {
        return ControllerUtils.doProcess(Roles.ROLE_PLATFORM_ADMIN, context, () -> {
            ...
            Components components = componentsService.findOne(COMPONENT_JUPYTERHUB);
            if(components == null) {
                components = new Components();
                components.setCompName(COMPONENT_JUPYTERHUB);
                componentsService.create(components);
            }

            // create jupyterhub custom resource.
            CustomResource jupyterhubCustomResource = CustomResourceUtils.fromYaml(decodedYaml);
            jupyterhubCustomResource.setComponents(components);
            k8sResourceService.createCustomResource(jupyterhubCustomResource);
        ...
        return ControllerUtils.doProcess(Roles.ROLE_PLATFORM_ADMIN, context, () -> {
            ...
            if(components != null) {
                for(CustomResource customResource : components.getCustomResourceSet()) {
                    ...
                    map.put("components", COMPONENT_JUPYTERHUB);
